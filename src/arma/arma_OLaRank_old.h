...
namespace armadillo {
  ...
  struct KeyEqual {
    bool operator()(const Key &lhs, const Key &rhs) const {
      return lhs.frameNumber_1 == rhs.frameNumber_1 &&
        lhs.frameNumber_2 == rhs.frameNumber_2;
    }
  };
  ...
  private:
    ...
    friend std::ostream &operator<<(std::ostream &, const armadillo::OLaRank_old &);

  public:
    armadillo::params parameters;
    armadillo::Kernel *svm_kernel;
    // size of the pattern

    int B;
    int m;
    vector<armadillo::supportData *> S;
    ...
    void setParameters(armadillo::params&, int &, int &, int &);
