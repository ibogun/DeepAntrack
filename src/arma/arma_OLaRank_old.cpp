...
armadillo::OLaRank_old::OLaRank_old(Kernel *svm_kernel_, int seed) {
    srand(seed);
 ...
 * @param verbose_
 */
void armadillo::OLaRank_old::setParameters(armadillo::params &learningParams,
                                           int &balance, int &m_,
                                           int &verbose_) {
...
rowvec armadillo::OLaRank_old::predictAll(mat &newX) {
 ...
 */
void armadillo::OLaRank_old::checkIfConstraintsSatisfied() {
    double sum = 0;
    for (int i = 0; i < this->S.size(); ++i) {
        ...
        for (int y = 0; y < this->S[i]->x->n_rows; ++y) {
            ...
            assert((*this->S[i]->beta)(y) <=
                   this->parameters.C * (this->S[i]->label == y));
            sum += (*this->S[i]->beta)(y);
        }
                ...
                if ((*S_i->beta)(y_bar) != 0) {
                    ...
                    grad(y) -=
                        ...
                        this->kernel_fast(*this->S[i]->x, *this->S[i]->y,
                                          (*this->S[i]->y).row(y_bar)(0),
                                          (*this->S[i]).frameNumber, newX,
                                          y_hat, y_hat.row(y)(0), frameNumber);

                    //					grad(y) -=
                    // this->S[i].beta(y_bar)
                    //                    * this->kernel(this->S[i].x,
                    //                    this->S[i].y.row(y_bar), newX,
                    //                    y_hat.row(y));
                }
            }
        }
 ...
 */
int armadillo::OLaRank_old::budgetMaintance() {
    int test = 0;
    ...
    // if the list is larger -> sort it first and delete starting from the
    // highest
    ...
    for (int i = static_cast<int>(idxToBeDeleted.size()) - 1; i >= 0; i--) {
    ...
    // std::cout<<"Number of  support Vectors : "<<test<<" out of:
    // "<<this->S.size()<<std::endl;
    int n = test;
    ...
    mat scores(static_cast<int>(this->S.size()), maxNRows, fill::ones);
    mat y_r, x_r;
    ...
    for (int i = 0; i < this->S.size(); ++i) {
        for (int k = 0; k < this->S[i]->x->n_rows; ++k) {
            if ((*this->S[i]->beta)(k) < 0) {
                ...
                scores(i, k) =
                    (pow((*this->S[i]->beta)(k), 2)) *
                    (this->kernel_fast(*this->S[i]->x, y_r, y_r.row(k)(0),
                                       (*this->S[i]).frameNumber,
                                       *this->S[i]->x, y_r, y_r.row(k)(0),
                                       (*this->S[i]).frameNumber) +
                     (this->kernel_fast(
                         *this->S[i]->x, y_r, y_r.row(this->S[i]->label)(0),
                         (*this->S[i]).frameNumber, *this->S[i]->x, y_r,
                         y_r.row(this->S[i]->label)(0),
                         (*this->S[i]).frameNumber)) -
    ...
    if (numberOfNegativeSP == 1) {
        // if there is only one negative support vector - delete the whole
        // pattern
        ...
        for (int z = 0; z < allSPtobeDeleted.size(); ++z) {
            ...
            for (int ii = 0; ii < this->S.size(); ++ii) {
                for (int yy = 0; yy < this->S[ii]->x->n_rows; ++yy) {
                    (*this->S[ii]->grad)(yy) +=
                        (*this->S[I]->beta)(idx)*kernel_fast(
                            *this->S[ii]->x, *this->S[ii]->y,
                            (*this->S[ii]->y).row(yy)(0),
                            (*this->S[ii]).frameNumber, *this->S[I]->x,
                            *this->S[I]->y, (*this->S[I]->y).row(idx)(0),
                            (*this->S[I]).frameNumber);
                }
            }
        }
        ...
        for (int ii = 0; ii < this->S.size(); ++ii) {
            for (int yy = 0; yy < this->S[ii]->x->n_rows; ++yy) {
                (*this->S[ii]->grad)(yy) -= (*this->S[I]->beta)(J)*kernel_fast(
                    *this->S[ii]->x, (*this->S[ii]->y),
                    (*this->S[ii]->y).row(yy)(0), (*this->S[ii]).frameNumber,
                    *this->S[I]->x, *this->S[I]->y,
                    (*this->S[I]->y).row(idx)(0), (*this->S[I]).frameNumber);
            }
        }
 ...
 * @param y_neg 	- y minus
 */
void armadillo::OLaRank_old::smoStep(const int i, mat &y_plus, mat &y_neg) {

    double k_00 = this->kernel_fast(
        *this->S[i]->x, *this->S[i]->y, y_plus(0), (*this->S[i]).frameNumber,
        *this->S[i]->x, *this->S[i]->y, y_plus(0), (*this->S[i]).frameNumber);
    double k_11 = this->kernel_fast(
        *this->S[i]->x, *this->S[i]->y, y_neg(0), (*this->S[i]).frameNumber,
        *this->S[i]->x, *this->S[i]->y, y_neg(0), (*this->S[i]).frameNumber);
    ...
    double lambda_u =
        ((*this->S[i]->grad)(y_plus(0)) - (*this->S[i]->grad)(y_neg(0))) /
        (k_00 + k_11 - 2 * k_01);
        ...
        for (int j = 0; j < this->S.size(); ++j) {
            ...
            for (int y = 0; y < this->S[j]->x->n_rows; ++y) {
                ...
                k_0 = this->kernel_fast(
                    *this->S[j]->x, *this->S[j]->y, (*this->S[j]->y).row(y)(0),
                    (*this->S[j]).frameNumber, *this->S[i]->x, *this->S[i]->x,
                    y_plus(0), (*this->S[i]).frameNumber);
                k_1 = this->kernel_fast(
                    *this->S[j]->x, *this->S[j]->y, (*this->S[j]->y).row(y)(0),
                    (*this->S[j]).frameNumber, *this->S[i]->x, *this->S[i]->y,
                    y_neg(0), (*this->S[i]).frameNumber);
 ...
 */
int armadillo::OLaRank_old::predict(mat &newX) {
            ...
            for (int yhat = 0; yhat < this->S[i]->x->n_rows; ++yhat) {
                ...
                if ((*this->S[i]->beta)(yhat) != 0) {
                    ...
                    current += (*this->S[i]->beta)(yhat) *
                               this->calculate_kernel(newX, y(0),
                                                      *this->S[i]->x, y_hat(0));
                }
            }
        }
 ...
 */
double armadillo::OLaRank_old::loss(const mat y, const mat y_hat) {
    ...
    double loss =
        1 - intersection / (y(3) * y(4) + y_hat(3) * y_hat(4) - intersection);
 ...
 */
double armadillo::OLaRank_old::calculateObjective() {
        ...
        for (int j = i; j < this->S.size(); ++j) {
            ...
            for (int y = 0; y < this->S[i]->x->n_rows; ++y) {
                for (int yhat = 0; yhat < this->S[j]->x->n_rows; ++yhat) {
                    ...
                    s = ((*S[i]->beta)(y) *
                         (*S[j]->beta)(yhat)*kernel(*S[i]->x, (*S[i]->y).row(y),
                                                    *S[j]->x,
                                                    (*S[i]->y).row(yhat)));
 ...
 * @return K(x,y,xp,yp);
 */
double armadillo::OLaRank_old::kernel(mat x, mat y, mat xp, mat yp) {

    double result = exp(-0.2 * norm(x.row(y(0)) - xp.row(yp(0))));
    return result;
}

double armadillo::OLaRank_old::kernel_fast(mat &x, mat &y_loc, int y,
                                           int frameNumber_1, mat &xp,
                                           mat &yp_loc, int yp,
                                           int frameNumber_2) {
    ...
    if (it == (this->kern).end()) {
        ...
        // set all elements to -infinity which means that the kernel value
        // wasn't calculated
        ...
        if ((it->second)(y, yp) != DBL_MIN) {
            ...
            // the value wasn't calculated. Firstly, calculate it and store in
            // the kernel
...
double armadillo::OLaRank_old::calculate_kernel(mat &x, int y, mat &xp,
                                                int yp) {
 ...
 */
armadillo::OLaRank_old::OLaRank_old(Kernel *svm_kernel_, params &learningParams,
 ...
 *
 * @param newX 				- 		pattern
 * @return 					- 		predicted label for
 * the
 * pattern
 */
int armadillo::OLaRank_old::processAndPredict(mat &newX, mat &newY,
                                              int frameNumber) {
    ...
    //
    //                std::cout<<"Gradient is not correct expected: "<<grad<<"
    //                got: "<<this->S[ii].grad(yy)<<std::endl;
    //            }
    //        }
    //    }
...
void armadillo::OLaRank_old::process(mat &newX, mat &y_hat, int y_hat_label,
    ...
    supportData *support = new supportData(
        newX, y_hat, y_hat_label, newX.size(), newX.n_rows, frameNumber);
    (*support->grad) = grad;
...
void armadillo::OLaRank_old::testIfObjectiveIncreases() {
 ...
 *  @param y 			first label
 */
void armadillo::OLaRank_old::initialize(mat &x, mat &y, const int label,
                                        int frameNumber) {
...
tuple<mat, vec> generateInput(const int &n, const int &m, const double step) {
    ...
    x.submat(0, 0, floor(n / 3) - 1, 0) =
        mat(floor(n / 3), 1, fill::ones) * (-1);
    x.submat(0, 1, floor(n / 3) - 1, 1) = mat(floor(n / 3), 1, fill::ones);

    x.submat(floor(n / 3), 0, 2 * floor(n / 3) - 1, 0) =
        mat(floor(n / 3), 1, fill::ones);
    x.submat(floor(n / 3), 1, 2 * floor(n / 3) - 1, 1) =
        mat(floor(n / 3), 1, fill::ones);

    y.subvec(floor(n / 3), 2 * floor(n / 3) - 1) =
        vec(floor(n / 3), fill::ones) * 2;

    x.submat(2 * floor(n / 3), 0, n - 1, 0) =
        mat(floor(n / 3), 1, fill::ones) * (-1);
    x.submat(2 * floor(n / 3), 1, n - 1, 1) =
        mat(floor(n / 3), 1, fill::ones) * (-1);

    y.subvec(2 * floor(n / 3), n - 1) =
        vec(n - 2 * floor(n / 3), fill::ones) * 3;
    ...
    for (int i = 0; i < n; ++i) {
        x.submat(i, 0, i, m - 1) =
            xcopy.submat(myvector[i], 0, myvector[i], m - 1);
...
double armadillo::OLaRank_old::recomputeGradient(int i, int y) {
    ...
    for (int j = 0; j < this->S.size(); ++j) {
        for (int yhat = 0; yhat < this->S[i]->x->n_rows; ++yhat) {
            grad -=
                (*this->S[j]->beta)(yhat) *
                this->kernel_fast(
                    *this->S[i]->x, *this->S[i]->y, (*this->S[i]->y).row(y)(0),
                    (*this->S[i]).frameNumber, *this->S[j]->x, *this->S[j]->y,
                    (*this->S[j]->y).row(yhat)(0), (*this->S[j]).frameNumber);
        }
    }
...
void armadillo::OLaRank_old::deleteKernelValues(int frameNumber) {
    ...
    for (int i = 0; i < this->S.size(); i++) {
        ...
        if (it != this->kern.end()) {
            // if the key is present. Delete the matrix associated with it from
            // the heap
...
std::ostream &operator<<(std::ostream &strm, const armadillo::OLaRank_old &s) {
...
armadillo::OLaRank_old::~OLaRank_old() {
