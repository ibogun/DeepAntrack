...
cv::Mat DeepFeatures::calculateFeature(cv::Mat &processedImage,
                                       std::vector<cv::Rect> &rects) {
  cv::Mat x = cv::Mat::zeros(static_cast<int> (rects.size()),
                                   this->calculateFeatureDimension(), CV_64F);
