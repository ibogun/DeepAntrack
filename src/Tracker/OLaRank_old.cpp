        ...
        for (int k = 0; k < this->S[i]->x.rows; ++k) {
            if ((this->S[i]->beta).at<double>(k) < 0) {
                ...
                scores.at<double>(i, k) =
                    ...
                    (this->kernel_fast(
                         this->S[i]->x, y_r, y_r.row(k).at<double>(0),
                         S_i_framenumber, this->S[i]->x, y_r,
                         y_r.row(k).at<double>(0), S_i_framenumber) +
                     (this->kernel_fast(
                         this->S[i]->x, y_r,
                         ...
                         y_r.row(this->S[i]->label).at<double>(0),
                         S_i_framenumber)) -
                     2 * this->kernel_fast(this->S[i]->x, y_r, y_r.row(this->S[i]->label).at<double>(0), S_i_framenumber, this->S[i]->x, y_r, y_r.row(k).at<double>(0), S_i_framenumber));
            }
        }
    }
            ...
            for (int ii = 0; ii < this->S.size(); ++ii) {
                for (int yy = 0; yy < this->S[ii]->x.rows; ++yy) {
                    (this->S[ii]->grad).at<double>(yy) +=
                        this->S[I]->beta.at<double>(idx) *
                        kernel_fast(this->S[ii]->x, this->S[ii]->y,
                                    (this->S[ii]->y).row(yy).at<double>(0),
                                    (this->S[ii])->frameNumber, this->S[I]->x,
                                    this->S[I]->y,
                                    (this->S[I]->y).row(idx).at<double>(0),
 ...
 * @param y_neg 	- y minus
 */
void OLaRank_old::smoStep(int i, const cv::Mat &y_plus, const cv::Mat &y_neg) {

    double k_00 = this->kernel_fast(
        this->S[i]->x, this->S[i]->y, y_plus.at<double>(0),
        (this->S[i])->frameNumber, this->S[i]->x, this->S[i]->y,
        y_plus.at<double>(0), (this->S[i])->frameNumber);
    ...
    double lambda_u = ((this->S[i]->grad).at<double>(y_plus.at<double>(0)) -
    ...
    double tmp2 =
        this->parameters.C * static_cast<int>(y_plus_0 == this->S[i]->label) -
    ...
    if (lambda != 0) {
        for (int j = 0; j < this->S.size(); ++j) {
            ...
            for (int y = 0; y < this->S[j]->x.rows; ++y) {
                ...
                k_0 = this->kernel_fast(this->S[j]->x, this->S[j]->y,
                                        (this->S[j]->y).row(y).at<double>(0),
                                        (this->S[j])->frameNumber,
                                        this->S[i]->x, this->S[i]->x, y_plus_0,
                                        (this->S[i])->frameNumber);
                k_1 = this->kernel_fast(this->S[j]->x, this->S[j]->y,
                                        (this->S[j]->y).row(y).at<double>(0),
                                        (this->S[j])->frameNumber,
                                        this->S[i]->x, this->S[i]->y, y_neg_0,
                                        (this->S[i])->frameNumber);
...
double OLaRank_old::loss(const cv::Mat &y, const cv::Mat &y_hat) {
    ...
    double loss = 1 -
                  intersection / (y.at<double>(3) * y.at<double>(4) +
                                  y_hat.at<double>(3) * y_hat.at<double>(4) -
                                  intersection);
...
double OLaRank_old::calculate_kernel(const cv::Mat &x, int y, const cv::Mat &xp,
                                     int yp) {
...
int OLaRank_old::processAndPredict(const cv::Mat &newX, const cv::Mat &newY,
    ...
    supportData *support =
        new supportData(newX, newY, y_hat_idx, 0, newX.rows, frameNumber);
    (support->grad) = grad;
...
void OLaRank_old::process(const cv::Mat &newX, const cv::Mat &y_hat,
                          int y_hat_label, int frameNumber) {
    ...
    // add new element into set S
    supportData *support =
        new supportData(newX, y_hat, y_hat_label, 0, newX.rows, frameNumber);
    (support->grad) = grad;
 ...
 *  @param y 			first label
 */
void OLaRank_old::initialize(const cv::Mat &x, const cv::Mat &y,
                             const int label, int frameNumber) {
        ...
        for (int yhat = 0; yhat < this->S[i]->x.rows; ++yhat) {
            grad -= (this->S[j]->beta).at<double>(yhat) *
                    this->kernel_fast(this->S[i]->x, this->S[i]->y,
                                      ...
                                      (this->S[i])->frameNumber, this->S[j]->x,
                                      this->S[j]->y,
